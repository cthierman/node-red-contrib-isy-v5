...
var ISY = function (controller_node, address, username, password, useHttps) {
    ...
    this.events.on('item_init_complete', function (isy) {
        if (isy.initialized.config && isy.initialized.nodeFolders && isy.initialized.nodes && isy.initialized.scenes && isy.initialized.programs && isy.initialized.int_variables && isy.initialized.state_variables && !isy.connected) {
            ...
            isy.watchdogTimer = setInterval(isy.websocketWatchdog.bind(isy), 90000);
// CET: Added Else so as to report if the above failed.
        } else {
	    isy.node.log('Failed to connect because one of the following were false');
	    isy.node.log('config:' + isy.initialized.config + 
...
// CET: added Jan 9th/2020
		this.node.log(' Got error on initialize websocket... Doing nothing as per original code. logging.');
                //do nothing
            }
...
// CET: added Dec 5/2019
//	    this.node.log('controlElement="'+controlElement+'" ActionValue='+
//		actionValue+' address="'+address+'" eventInfo="'+eventInfo+'"');
...
// CET: Dec 5/2019
//this.node.log('action_type="'+action_type+'" Control element update received');

                if (action_type != 'ERR') {
                    var uom = document.childNamed('action').attr.uom || '';
                    var prec = document.childNamed('action').attr.prec || '';
// CET: Dec 5/2019
//this.node.log('uom="'+uom+'" prec="'+prec+'"');
                    try {
        ...
        restler.get(this.protocol + '://' + this.address + '/rest/config', this.options).on('complete', function (result, response) {
            try {
                if (isy.checkForFailure(response)) {
                    isy.node.error("Unable to contact the ISY to get the configuration (getISYConfig): " + result.message);
                }
                else {
...
//isy.node.log('DEBUG line 463 (result):' + result );
			...
			if (document.childNamed('nodedefs') !== undefined ) {
                ...
                isy.node.error("Unable to contact the ISY to get the configuration (catch GetISYConfig): " + err);
            }
        }).on('error', function (err, response) {
            isy.node.error("Unable to contact the ISY to get the configuration (on error GetISYConfig): " + err);
...
// CET: May 25/2020 Commented out since restler doesn't seem to clear timeouts.
...
//            isy.node.error("ISY-JS: Timed out contacting ISY for the configuration");
    ...
    this.getNodesAndScenes = function () {
        ...
        //Parse known nodes and scenes:
// CET: May 25th 2020 
// added log line below.
	isy.node.log("getNodesAndScenes: About to send: '"+ this.protocol + "://" + this.address + "/rest/nodes'");
...
// CET: May 25th 2020 added this for debugging unindented so it could be easily removed.
isy.node.log("getNodesAndScenes: About to call _loadNodeFolders");

                    isy._loadNodeFolders(document);

isy.node.log("getNodesAndScenes: About to call _loadNodes");

                    isy._loadNodes(document);

isy.node.log("getNodesAndScenes: About to call _loadScenes");

                    isy._loadScenes(document);

isy.node.log("getNodesAndScenes: done.. getNodesAndScenes");
// trying to force a clear ... this doesn't appear to work .. CET: May 25/2020
...
// CET: May 25/2020... Even though I get a successful completion from the above code, restler still timesout
// after 50000s. Since it's ignorable. I will comment out the timeouts. sigh for now.
//        }).on('timeout', function (ms) {
//            isy.node.error("ISY-JS: getNodesAndScenes: Timed out after "+ ms +" contacting ISY for the list of nodes");
        });
    }
...
// May 25/2020
// CET: since restler doesn't seem to cancel timeouts. I'm going to ignore for now.
//        }).on('timeout', function (ms) {
...
// CET: Added IF and Else to check if variables are even defined.
...
// CET: May 25/2020 I believe I should uncomment the code below? 
                    //if (type == 1) {
                    //    isy.initialized.int_variables = true;
                    //} else if (type == 2) {
                    //    isy.initialized.state_variables = true;
...
// May 25/2020 Since it would seem restler isn't clearing the timeout when we are successful. I'm commenting this out.
//        }).on('timeout', function (ms) {
...
// May 25/2020:
// CET: since it would seem restler doesn't reset timeouts... commenting out.
//        }).on('timeout', function (ms) {
