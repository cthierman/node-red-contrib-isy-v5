...
ISYNode.prototype.runCmd = function (cmd, params = []) {
    try {
        ...
        this.controller.node.log('Command received. cmd: ' + cmd.toString() + ', params: ' + params.toString());

// CET: Added If == Query to handle Queries since it is not part of command from the Rest documentation
        var url ="";
	if ( cmd == "QUERY" ) {
// CET: additonal mod May 24th/2020 to accomodate V5 of rest api
//		url = '/rest/status/' + this.address; 
		url = '/rest/status';
        	this.controller.node.log('Sending command (' + cmd + ') as url ('+url+') ');
        	this.controller.REST(url, this.controller.nodeDefStatus);
        	this.events.emit('property_updated', 'ST' );
...
// CET: commented out, and moved to inside the if above. So that it's only run as a runcmd.
//        this.controller.node.log('Sending command (' + cmd + ') as url ('+url+') ');
//        this.controller.REST(url);
    } catch (err) {
